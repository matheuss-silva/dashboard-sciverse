    ...
    if (query.length == 0 || query.length >= 3) {
        $.get(ajaxSearchUrl, { q: query }, function(data) {
            ...
            for (let i = 0; i < data.length; i++) {
                let item = data[i];
                tbody.append(`<tr class="table table-striped">
                    <td>${item.id}</td>  // Django usa minúsculas para nomes de campo
                    <td>${item.titulo}</td>
                    <td>${item.nome_do_curso}</td>
                    <td>${item.nome_autor_principal}</td>  // Ajuste de acordo com o seu modelo
